...
print("WS HUB loaded successfully")
...
function applyTheme(themeName)
    ...
    task.spawn(function()
        ...
        if buildMiniActionsUI then
            ...
            local ok, err = pcall(buildMiniActionsUI)
            if not ok then
                warn("WS HUB: buildMiniActionsUI failed:", err)
            else
                task.wait()
        ...
        if _G.buildQuickActionsDock then
            local oldDock = PlayerGui:FindFirstChild("WSQuickActions")
            local wasDockVisible = oldDock and oldDock:FindFirstChild("DockPanel") and oldDock.DockPanel.Visible
            local ok2, err2 = pcall(_G.buildQuickActionsDock)
            if not ok2 then
                warn("WS HUB: buildQuickActionsDock failed:", err2)
            else
                task.wait()
                local newDock = PlayerGui:FindFirstChild("WSQuickActions")
                local newPanel = newDock and newDock:FindFirstChild("DockPanel")
                if newPanel and wasDockVisible == false then
                    newPanel.Visible = false
                end
            end
        end

        local guisRT = {"AutoStealUI","XiAdminPanel","SettingsUI","BullysSettings","BullysStatusHUD","BullysAutoBuyUI","BullysMiniActions","WSQuickActions"}
...
local function sanitizeGuiToggleText(raw)
...
local _noOnOffGuiNames = {
    AutoStealUI = true,
    XiAdminPanel = true,
    XiAdminToolsPanel = true,
    SettingsUI = true,
    BullysSettings = true,
    BullysMiniActions = true,
    BullysAutoBuyUI = true,
    BullysStatusHUD = true,
    WSMiniDesync = true,
    WSQuickActions = true,
}
local _noOnOffAttached = {}
local function attachNoOnOffSanitizerToGui(sg)
    if not sg or not sg:IsA("ScreenGui") then return end
    if _noOnOffAttached[sg] then return end
    _noOnOffAttached[sg] = true

    local function hookTextObj(obj)
        if not (obj:IsA("TextLabel") or obj:IsA("TextButton")) then return end
        local applying = false
        local function applyClean()
            if applying then return end
            local before = obj.Text
            local after = sanitizeGuiToggleText(before)
        ...
        applyClean()
        obj:GetPropertyChangedSignal("Text"):Connect(applyClean)
    end

    for _, d in ipairs(sg:GetDescendants()) do hookTextObj(d) end
    sg.DescendantAdded:Connect(hookTextObj)
end

task.spawn(function()
    local function bindCurrent()
        for _, ch in ipairs(PlayerGui:GetChildren()) do
            if _noOnOffGuiNames[ch.Name] then
                attachNoOnOffSanitizerToGui(ch)
            end
        end
    end
    bindCurrent()
...
task.spawn(function()
    ...
    UI.AutoStealUI = screenGui

    local mobileScale = IS_MOBILE and 0.75 or 1
    local frame = Instance.new("Frame")
    frame.Size = UDim2.new(0, 276 * mobileScale, 0, 408 * mobileScale)
    ...
    miniCard.Size = UDim2.new(0, 276 * mobileScale, 0, 72)
    ...
    if IS_MOBILE then setMinimized(true) end
    ...
    local function executeInstantSteal(prompt)
        ...
        ShowNotification("INSTANT STEAL", "Roubo instantâneo executado")
...
local function executeReset()
    if _resetBusy then
        ShowNotification("reset", "aguarde...")
        return
    end
    ...
    task.spawn(function()
        ...
        if oldHum then
            pcall(function() oldHum.Health = 0 end)
            pcall(function() oldHum:ChangeState(Enum.HumanoidStateType.Dead) end)
        end

        local okLoad = pcall(function()
            if plr.Character == oldChar then
                plr:LoadCharacter()
            end
        end)
        ...
        if not okLoad then
            ShowNotification("reset", "falhou")
        else
            ShowNotification("reset", "timeout")
        end
        _resetBusy = false
    end)
end

local function isInMyBase(maxDist)
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return false end
    local plots = Workspace:FindFirstChild("Plots")
    if not plots then return false end
    local maxD = tonumber(maxDist) or 120
    for _, plot in ipairs(plots:GetChildren()) do
        local sign = plot:FindFirstChild("PlotSign")
        local yb = sign and sign:FindFirstChild("YourBase")
...
local function doTpAction()
    ...
    ShowNotification("TP", "Teleportando para o melhor alvo")
...
local function doCloneAction()
    if _G._activePinConn then _G._activePinConn:Disconnect(); _G._activePinConn = nil end
    if _G._activePlat and _G._activePlat.Parent then _G._activePlat:Destroy(); _G._activePlat = nil end
    ShowNotification("CLONE", "Clonando personagem")
    if isInMyBase(120) then
        task.spawn(function()
            instantClone()
            while _G.isCloning do task.wait() end
            doTpAction()
...
local function toggleCarpetAction()
...
local function toggleSettingsPanelAction()
    if _G.BullysSettingsUI and _G.BullysSettingsUI.panel then
        local panel = _G.BullysSettingsUI.panel
        panel.Visible = not panel.Visible
        return
    end
    if _G.NotzhubSettingsUI and _G.NotzhubSettingsUI.panel then
        local panel = _G.NotzhubSettingsUI.panel
        panel.Visible = not panel.Visible
        return
    end
    local legacy = PlayerGui:FindFirstChild("SettingsUI")
    if legacy then legacy.Enabled = not legacy.Enabled end
end

local function toggleMiniActionsPanelAction()
    local gui = PlayerGui:FindFirstChild("BullysMiniActions")
    local panel = gui and gui:FindFirstChild("MiniPanel")
    if panel then
        panel.Visible = not panel.Visible
        Config.ShowMiniActions = panel.Visible
        SaveConfig()
    end
end

local function toggleAutoBuyPanelAction()
    if _G.toggleAutoBuyPanel then
        pcall(_G.toggleAutoBuyPanel)
        return
    end
    local gui = PlayerGui:FindFirstChild("BullysAutoBuyUI")
    local panel = gui and gui:FindFirstChild("ABPanel")
    if panel then
        panel.Visible = not panel.Visible
        Config.HideAutoBuyUI = not panel.Visible
        SaveConfig()
    end
end

local function toggleQuickDockAction()
    local gui = PlayerGui:FindFirstChild("WSQuickActions")
    local panel = gui and gui:FindFirstChild("DockPanel")
    if (not panel) and _G.buildQuickActionsDock then
        if Config.ShowQuickDock == false then
            Config.ShowQuickDock = true
            SaveConfig()
        end
        _G.buildQuickActionsDock()
        gui = PlayerGui:FindFirstChild("WSQuickActions")
        panel = gui and gui:FindFirstChild("DockPanel")
    end
    if panel then
        panel.Visible = not panel.Visible
    end
...
_G.wsDoTpAction = doTpAction
_G.wsDoCloneAction = doCloneAction
_G.wsToggleCarpetAction = toggleCarpetAction
_G.wsToggleSettingsPanelAction = toggleSettingsPanelAction
_G.wsToggleMiniActionsPanelAction = toggleMiniActionsPanelAction
_G.wsToggleAutoBuyPanelAction = toggleAutoBuyPanelAction
_G.wsToggleQuickDockAction = toggleQuickDockAction
_G.wsExecuteReset = executeReset
...
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    local miniPanelKey = safeKeyCode(Config.MiniPanelKey, Enum.KeyCode.M)
    local autoBuyPanelKey = safeKeyCode(Config.AutoBuyPanelKey, Enum.KeyCode.N)
    local remoteSellKey = safeKeyCode(Config.RemoteSellKeybind, Enum.KeyCode.O)
    local smartUnlockKey = safeKeyCode(Config.SmartUnlockKeybind, Enum.KeyCode.U)
    local silentModeKey = safeKeyCode(Config.SilentModeKeybind, Enum.KeyCode.Y)
    local quickDockKey = safeKeyCode(Config.QuickDockKey, Enum.KeyCode.H)
    ...
    if input.KeyCode == miniPanelKey then
        toggleMiniActionsPanelAction()
    end

    if input.KeyCode == autoBuyPanelKey then
        toggleAutoBuyPanelAction()
    end

    if input.KeyCode == remoteSellKey and _G.toggleRemoteSell then
        pcall(_G.toggleRemoteSell)
    end

    if input.KeyCode == smartUnlockKey and _G.doSmartUnlock then
        task.spawn(_G.doSmartUnlock)
    end

    if input.KeyCode == silentModeKey and _G.toggleSilentMode then
        pcall(_G.toggleSilentMode)
    end

    if input.KeyCode == quickDockKey then
        toggleQuickDockAction()
    end

    -- GUI Toggles
    if input.KeyCode == Enum.KeyCode.F1 then
        if UI.AutoStealUI then UI.AutoStealUI.Enabled = not UI.AutoStealUI.Enabled end
    end
    if input.KeyCode == Enum.KeyCode.F2 then
        if UI.AdminPanel then UI.AdminPanel.Enabled = not UI.AdminPanel.Enabled end
    end
    if input.KeyCode == Enum.KeyCode.F3 then
        if UI.settingsGui then UI.settingsGui.Enabled = not UI.settingsGui.Enabled end
    end
end)
...
local mobileScale = IS_MOBILE and 0.8 or 1
sFrame.Size = UDim2.new(0, 300 * mobileScale, 0, 650 * mobileScale)
...
local bTp = Instance.new("TextButton", rTpKey)
bTp.Size=UDim2.new(0,60,0,24); bTp.Position=UDim2.new(1,-70,0.5,-12)
bTp.BackgroundColor3=Theme.SurfaceHighlight; bTp.Text=Config.TpSettings.TpKey or "T"
bTp.Font=Enum.Font.GothamBold; bTp.TextColor3=Theme.TextPrimary; bTp.TextSize=12
Instance.new("UICorner",bTp).CornerRadius=UDim.new(0,4)
bTp.MouseButton1Click:Connect(function()
    bTp.Text="..."; bTp.TextColor3=Theme.Accent1
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
            Config.TpSettings.TpKey=inp.KeyCode.Name; bTp.Text=inp.KeyCode.Name
            bTp.TextColor3=Theme.TextPrimary; SaveConfig(); con:Disconnect()
            ShowNotification("TP KEYBIND", inp.KeyCode.Name)
        end
    end)
end)
...
local bClone = Instance.new("TextButton", rCloneKey)
bClone.Size=UDim2.new(0,60,0,24); bClone.Position=UDim2.new(1,-70,0.5,-12)
bClone.BackgroundColor3=Theme.SurfaceHighlight; bClone.Text=Config.TpSettings.CloneKey or "V"
bClone.Font=Enum.Font.GothamBold; bClone.TextColor3=Theme.TextPrimary; bClone.TextSize=12
Instance.new("UICorner",bClone).CornerRadius=UDim.new(0,4)
bClone.MouseButton1Click:Connect(function()
    bClone.Text="..."; bClone.TextColor3=Theme.Accent1
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
            Config.TpSettings.CloneKey=inp.KeyCode.Name; bClone.Text=inp.KeyCode.Name
            bClone.TextColor3=Theme.TextPrimary; SaveConfig(); con:Disconnect()
            ShowNotification("CLONE KEYBIND", inp.KeyCode.Name)
        end
    end)
end)

local rResetKey = CreateRow("Reset Keybind")
local bReset = Instance.new("TextButton", rResetKey)
bReset.Size=UDim2.new(0,60,0,24); bReset.Position=UDim2.new(1,-70,0.5,-12)
bReset.BackgroundColor3=Theme.SurfaceHighlight; bReset.Text=Config.ResetKey or "X"
...
bReset.MouseButton1Click:Connect(function()
    bReset.Text="..."; bReset.TextColor3=Theme.Accent1
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
            Config.ResetKey=inp.KeyCode.Name; bReset.Text=inp.KeyCode.Name
            bReset.TextColor3=Theme.TextPrimary; SaveConfig(); con:Disconnect()
            ShowNotification("RESET KEYBIND", inp.KeyCode.Name)
        end
    end)
end)

local rMiniPanelKey = CreateRow("Mini Panel Keybind")
local bMiniPanel = Instance.new("TextButton", rMiniPanelKey)
bMiniPanel.Size=UDim2.new(0,60,0,24); bMiniPanel.Position=UDim2.new(1,-70,0.5,-12)
bMiniPanel.BackgroundColor3=Theme.SurfaceHighlight; bMiniPanel.Text=Config.MiniPanelKey or "M"
bMiniPanel.Font=Enum.Font.GothamBold; bMiniPanel.TextColor3=Theme.TextPrimary; bMiniPanel.TextSize=12
Instance.new("UICorner",bMiniPanel).CornerRadius=UDim.new(0,4)
bMiniPanel.MouseButton1Click:Connect(function()
    bMiniPanel.Text="..."; bMiniPanel.TextColor3=Theme.Accent1
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
            Config.MiniPanelKey=inp.KeyCode.Name; bMiniPanel.Text=inp.KeyCode.Name
            bMiniPanel.TextColor3=Theme.TextPrimary; SaveConfig(); con:Disconnect()
            ShowNotification("MINI PANEL KEYBIND", inp.KeyCode.Name)
        end
    end)
end)

local rAutoBuyKey = CreateRow("Auto Buy Panel Keybind")
local bAutoBuy = Instance.new("TextButton", rAutoBuyKey)
bAutoBuy.Size=UDim2.new(0,60,0,24); bAutoBuy.Position=UDim2.new(1,-70,0.5,-12)
bAutoBuy.BackgroundColor3=Theme.SurfaceHighlight; bAutoBuy.Text=Config.AutoBuyPanelKey or "N"
bAutoBuy.Font=Enum.Font.GothamBold; bAutoBuy.TextColor3=Theme.TextPrimary; bAutoBuy.TextSize=12
Instance.new("UICorner",bAutoBuy).CornerRadius=UDim.new(0,4)
bAutoBuy.MouseButton1Click:Connect(function()
    bAutoBuy.Text="..."; bAutoBuy.TextColor3=Theme.Accent1
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
            Config.AutoBuyPanelKey=inp.KeyCode.Name; bAutoBuy.Text=inp.KeyCode.Name
            bAutoBuy.TextColor3=Theme.TextPrimary; SaveConfig(); con:Disconnect()
            ShowNotification("AUTO BUY KEYBIND", inp.KeyCode.Name)
        end
    end)
end)

local rRemoteSellKey = CreateRow("Remote Sell Keybind")
local bRemoteSell = Instance.new("TextButton", rRemoteSellKey)
bRemoteSell.Size=UDim2.new(0,60,0,24); bRemoteSell.Position=UDim2.new(1,-70,0.5,-12)
bRemoteSell.BackgroundColor3=Theme.SurfaceHighlight; bRemoteSell.Text=Config.RemoteSellKeybind or "O"
bRemoteSell.Font=Enum.Font.GothamBold; bRemoteSell.TextColor3=Theme.TextPrimary; bRemoteSell.TextSize=12
Instance.new("UICorner",bRemoteSell).CornerRadius=UDim.new(0,4)
bRemoteSell.MouseButton1Click:Connect(function()
    bRemoteSell.Text="..."; bRemoteSell.TextColor3=Theme.Accent1
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
            Config.RemoteSellKeybind=inp.KeyCode.Name; bRemoteSell.Text=inp.KeyCode.Name
            bRemoteSell.TextColor3=Theme.TextPrimary; SaveConfig(); con:Disconnect()
            ShowNotification("REMOTE SELL KEYBIND", inp.KeyCode.Name)
        end
    end)
end)

local rSmartUnlockKey = CreateRow("Smart Unlock Keybind")
local bSmartUnlock = Instance.new("TextButton", rSmartUnlockKey)
bSmartUnlock.Size=UDim2.new(0,60,0,24); bSmartUnlock.Position=UDim2.new(1,-70,0.5,-12)
bSmartUnlock.BackgroundColor3=Theme.SurfaceHighlight; bSmartUnlock.Text=Config.SmartUnlockKeybind or "U"
bSmartUnlock.Font=Enum.Font.GothamBold; bSmartUnlock.TextColor3=Theme.TextPrimary; bSmartUnlock.TextSize=12
Instance.new("UICorner",bSmartUnlock).CornerRadius=UDim.new(0,4)
bSmartUnlock.MouseButton1Click:Connect(function()
    bSmartUnlock.Text="..."; bSmartUnlock.TextColor3=Theme.Accent1
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
            Config.SmartUnlockKeybind=inp.KeyCode.Name; bSmartUnlock.Text=inp.KeyCode.Name
            bSmartUnlock.TextColor3=Theme.TextPrimary; SaveConfig(); con:Disconnect()
            ShowNotification("SMART UNLOCK KEYBIND", inp.KeyCode.Name)
        end
    end)
end)

local rSilentModeKey = CreateRow("Silent Mode Keybind")
local bSilentMode = Instance.new("TextButton", rSilentModeKey)
bSilentMode.Size=UDim2.new(0,60,0,24); bSilentMode.Position=UDim2.new(1,-70,0.5,-12)
bSilentMode.BackgroundColor3=Theme.SurfaceHighlight; bSilentMode.Text=Config.SilentModeKeybind or "Y"
bSilentMode.Font=Enum.Font.GothamBold; bSilentMode.TextColor3=Theme.TextPrimary; bSilentMode.TextSize=12
Instance.new("UICorner",bSilentMode).CornerRadius=UDim.new(0,4)
bSilentMode.MouseButton1Click:Connect(function()
    bSilentMode.Text="..."; bSilentMode.TextColor3=Theme.Accent1
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
            Config.SilentModeKeybind=inp.KeyCode.Name; bSilentMode.Text=inp.KeyCode.Name
            bSilentMode.TextColor3=Theme.TextPrimary; SaveConfig(); con:Disconnect()
            ShowNotification("SILENT MODE KEYBIND", inp.KeyCode.Name)
        end
    end)
end)

local rQuickDockKey = CreateRow("Quick Dock Keybind")
local bQuickDock = Instance.new("TextButton", rQuickDockKey)
bQuickDock.Size=UDim2.new(0,60,0,24); bQuickDock.Position=UDim2.new(1,-70,0.5,-12)
bQuickDock.BackgroundColor3=Theme.SurfaceHighlight; bQuickDock.Text=Config.QuickDockKey or "H"
bQuickDock.Font=Enum.Font.GothamBold; bQuickDock.TextColor3=Theme.TextPrimary; bQuickDock.TextSize=12
Instance.new("UICorner",bQuickDock).CornerRadius=UDim.new(0,4)
bQuickDock.MouseButton1Click:Connect(function()
    bQuickDock.Text="..."; bQuickDock.TextColor3=Theme.Accent1
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
            Config.QuickDockKey=inp.KeyCode.Name; bQuickDock.Text=inp.KeyCode.Name
            bQuickDock.TextColor3=Theme.TextPrimary; SaveConfig(); con:Disconnect()
            ShowNotification("QUICK DOCK KEYBIND", inp.KeyCode.Name)
        end
    end)
end)
...
function buildBullysSettingsUI()
    ...
    makeKey(mS,"Mini UI Toggle Key",function() return Config.MiniPanelKey or "M" end,function(v) Config.MiniPanelKey=v; SaveConfig() end,51)
    makeKey(mS,"Auto Buy Panel Key",function() return Config.AutoBuyPanelKey or "N" end,function(v) Config.AutoBuyPanelKey=v; SaveConfig() end,52)
    makeKey(mS,"Remote Sell Key",function() return Config.RemoteSellKeybind or "O" end,function(v) Config.RemoteSellKeybind=v; SaveConfig() end,53)
    makeKey(mS,"Smart Unlock Key",function() return Config.SmartUnlockKeybind or "U" end,function(v) Config.SmartUnlockKeybind=v; SaveConfig() end,54)
    makeKey(mS,"Silent Mode Key",function() return Config.SilentModeKeybind or "Y" end,function(v) Config.SilentModeKeybind=v; SaveConfig() end,55)
    makeKey(mS,"Quick Dock Key",function() return Config.QuickDockKey or "H" end,function(v) Config.QuickDockKey=v; SaveConfig() end,56)
    do
        local rRejoin=Instance.new("Frame",mS); rRejoin.Size=UDim2.new(1,0,0,36); rRejoin.BackgroundColor3=C().SURF; rRejoin.BorderSizePixel=0; rRejoin.LayoutOrder=60; Instance.new("UICorner",rRejoin).CornerRadius=UDim.new(0,7)
    ...
    makeToggle(uhS,"Show Quick Dock",function() return Config.ShowQuickDock ~= false end,function(v) Config.ShowQuickDock=v; SaveConfig(); if _G.buildQuickActionsDock then _G.buildQuickActionsDock() end end,21)
...
-- QUICK ACTIONS DOCK (mobile + pc)
local function buildQuickActionsDock()
    local oldLegacy = PlayerGui:FindFirstChild("NotzhubMobileButtons")
    if oldLegacy then oldLegacy:Destroy() end

    local oldGui = PlayerGui:FindFirstChild("WSQuickActions")
    if oldGui then oldGui:Destroy() end

    if Config.ShowQuickDock == false then return end

    local dockGui = Instance.new("ScreenGui")
    dockGui.Name = "WSQuickActions"
    dockGui.ResetOnSpawn = false
    dockGui.DisplayOrder = 55
    dockGui.Parent = PlayerGui

    local panelW = IS_MOBILE and 236 or 220
    local panelH = IS_MOBILE and 276 or 246

    local panel = Instance.new("Frame", dockGui)
    panel.Name = "DockPanel"
    panel.Size = UDim2.new(0, panelW, 0, panelH)
    local p = (Config.Positions and Config.Positions.QuickDock) or {X = 0.84, Y = 0.28}
    panel.Position = UDim2.new(p.X or 0.84, 0, p.Y or 0.28, 0)
    ...
    header.BackgroundTransparency = 1
    MakeDraggable(header, panel, "QuickDock")
    ...
    collapseBtn.Position = UDim2.new(1, -28, 0.5, -11)
    collapseBtn.BackgroundColor3 = Theme.SurfaceHighlight
    collapseBtn.TextColor3 = Theme.TextPrimary
    ...
    div.Position = UDim2.new(0, 6, 0, 34)
    ...
    grid.CellSize = UDim2.new(0, (panelW - 22) / 2, 0, 30)
    grid.SortOrder = Enum.SortOrder.LayoutOrder

    local function setCollapsed(on)
        Config.QuickDockCollapsed = on and true or false
        SaveConfig()
        body.Visible = not Config.QuickDockCollapsed
        panel.Size = UDim2.new(0, panelW, 0, Config.QuickDockCollapsed and 38 or panelH)
        collapseBtn.Text = Config.QuickDockCollapsed and "+" or "-"
    end

    collapseBtn.MouseButton1Click:Connect(function()
        setCollapsed(not Config.QuickDockCollapsed)
    end)

    local function setButtonState(btn, on)
        btn.BackgroundColor3 = on and Theme.Accent1 or Theme.Surface
        btn.TextColor3 = on and Color3.new(0, 0, 0) or Theme.TextPrimary
    end

    local dynamicButtons = {}
    local function makeActionBtn(order, label, keyGetter, callback, stateGetter)
        ...
        bst.Color = Theme.SurfaceHighlight
        bst.Thickness = 1
        bst.Transparency = 0.45

        local function updateText()
            local key = ""
            if keyGetter then key = tostring(keyGetter() or "") end
        ...
        if stateGetter then
            dynamicButtons[#dynamicButtons + 1] = function()
                updateText()
                local ok, on = pcall(stateGetter)
                if ok then
                    setButtonState(btn, on and true or false)
                end
            end
        else
            dynamicButtons[#dynamicButtons + 1] = updateText
        end

        return btn
    end

    makeActionBtn(1,  "TP",        function() return Config.TpSettings.TpKey end, doTpAction)
    makeActionBtn(2,  "CLONE",     function() return Config.TpSettings.CloneKey end, doCloneAction)
    makeActionBtn(3,  "CARPET",    function() return Config.TpSettings.CarpetSpeedKey end, toggleCarpetAction, function() return State.carpetSpeedEnabled end)
    makeActionBtn(4,  "RESET",     function() return Config.ResetKey end, executeReset)
    makeActionBtn(5,  "SETTINGS",  function() return Config.MenuKey end, toggleSettingsPanelAction)
    makeActionBtn(6,  "MINI UI",   function() return Config.MiniPanelKey end, toggleMiniActionsPanelAction, function()
        local g = PlayerGui:FindFirstChild("BullysMiniActions")
        local mp = g and g:FindFirstChild("MiniPanel")
        return mp and mp.Visible
    end)
    makeActionBtn(7,  "AUTO BUY",  function() return Config.AutoBuyKey end, function() if _G.toggleAutoBuy then _G.toggleAutoBuy() end end, function() return _G.getAutoBuyActive and _G.getAutoBuyActive() end)
    makeActionBtn(8,  "AB PANEL",  function() return Config.AutoBuyPanelKey end, toggleAutoBuyPanelAction, function()
        local g = PlayerGui:FindFirstChild("BullysAutoBuyUI")
        local p2 = g and g:FindFirstChild("ABPanel")
        return p2 and p2.Visible
    end)
    makeActionBtn(9,  "SELL",      function() return Config.RemoteSellKeybind end, function() if _G.toggleRemoteSell then _G.toggleRemoteSell() end end, function() return _G.getRemoteSellVisible and _G.getRemoteSellVisible() end)
    makeActionBtn(10, "UNLOCK",    function() return Config.SmartUnlockKeybind end, function() if _G.doSmartUnlock then task.spawn(_G.doSmartUnlock) end end)
    makeActionBtn(11, "SILENT",    function() return Config.SilentModeKeybind end, function() if _G.toggleSilentMode then _G.toggleSilentMode() end end, function() return _G.getSilentMode and _G.getSilentMode() end)

    setCollapsed(Config.QuickDockCollapsed == true)

    task.spawn(function()
        while dockGui and dockGui.Parent do
            for _, fn in ipairs(dynamicButtons) do
                pcall(fn)
            end
            task.wait(0.35)
        end
    ...
    _G.toggleQuickDockPanel = function()
        panel.Visible = not panel.Visible
...
_G.buildQuickActionsDock = buildQuickActionsDock

task.spawn(function()
    task.wait(2)
    buildQuickActionsDock()
end)
task.spawn(function()
    task.wait(1.5)
    if Config.CurrentTheme and THEMES and THEMES[Config.CurrentTheme] then
...
function buildMiniActionsUI()
    ...
    panel.BackgroundTransparency = 0.08
    panel.BorderSizePixel = 0
    panel.Visible = (Config.ShowMiniActions ~= false) and not Config.AutoHideMiniUI
    Instance.new("UICorner", panel).CornerRadius = UDim.new(0, 10)
    ...
    local invisBtn = mBtn("Invisible Steal", 21, Theme.Surface, function() end)
    local function updInvisMini()
        local on = _G.invisibleStealEnabled
        invisBtn.Text = "Invisible Steal"
    ...
    -- ── AUTO ────────────────────────────────────────────────────────────
    miniSec("AUTO", 30)
    local abBackBtn = mBtn("Auto Back", 31, Theme.Surface, function() end)
    local function updABMini()
        local on = Config.AutoBack
        abBackBtn.Text = "Auto Back"
    ...
    do
        local akBtn = mBtn("Auto Kick", 32, Theme.Surface, function() end)

        local function updateAKBtn()
            local on = Config.AutoKickOnSteal
            akBtn.Text = "Auto Kick"
    ...
    _G.toggleAutoBuyPanel = function(forceState)
        local nextState = abPanel.Visible
        if type(forceState) == "boolean" then
            nextState = forceState
        else
            nextState = not nextState
        end
        abPanel.Visible = nextState
        Config.HideAutoBuyUI = not nextState
        SaveConfig()
        return nextState
    end
    ...
    -- mini gui button
    local autoBuyBtn = mBtn("Auto Buy", 7, Theme.Surface, function() end)
    local function syncAutoBuyVisuals()
    ...
    _G.toggleAutoBuy = function()
        toggleAutoBuy()
        syncAutoBuyVisuals()
        return autoBuyActive
    end
    _G.getAutoBuyActive = function()
        return autoBuyActive
    end
    ...
    local carpetMiniBtn = mBtn("Carpet Speed", 51, Theme.Surface, function() end)
    local function updCarpetMini()
        local on = State.carpetSpeedEnabled
        carpetMiniBtn.Text             = "Carpet Speed"
    ...
    local ijMiniBtn = mBtn("Infinite Jump", 52, Theme.Surface, function() end)
    local function updIJMini()
        local on = State.infiniteJumpEnabled
        ijMiniBtn.Text             = "Infinite Jump"
    ...
    local rsBtn = mBtn("Remote Sell", 61, Theme.Surface, function() end)
    ...
    _G.getRemoteSellVisible = function()
        return rsVisible
    end
    _G.toggleMiniActionsPanel = function(forceState)
        local nextState = panel.Visible
        if type(forceState) == "boolean" then
            nextState = forceState
        else
            nextState = not nextState
        ...
        Config.ShowMiniActions = nextState
        SaveConfig()
        return nextState
    end
    _G.getMiniActionsVisible = function()
        return panel.Visible
    end
...
task.spawn(function()
    buildMiniActionsUI()
    -- Auto hide on start
    if Config.AutoHideMiniUI or Config.ShowMiniActions == false then
        local g = PlayerGui:FindFirstChild("BullysMiniActions")
...
local function _findRaknet()
    ...
    if typeof(getgenv) == "function" then
        local genv = getgenv()
        if typeof(genv.raknet) == "table" then return genv.raknet end
    end
    if typeof(Fluxus) == "table" and typeof(Fluxus.raknet) == "table" then return Fluxus.raknet end
    if typeof(Krnl) == "table" and typeof(Krnl.raknet) == "table" then return Krnl.raknet end
    if typeof(Delta) == "table" and typeof(Delta.raknet) == "table" then return Delta.raknet end
    -- Fallback for other executors
    for _, env in ipairs({getfenv(0), _G, shared}) do
        if typeof(env) == "table" and typeof(env.raknet) == "table" then return env.raknet end
    end
    return nil
end
...
local function setRaknetDesync(enabled)
    ...
    if enabled and not _raknetHookRegistered then 
        status = "SEM RAKNET" 
        if IS_MOBILE then status = "NAO SUPORTADO NO MOBILE" end
    end
...
task.spawn(function()
    while true do
        task.wait(5)
        if _raknetDesyncActive and not _raknetHookRegistered then
            pcall(initRaknetHook)
        end
    end
end)
