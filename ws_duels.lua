...
local ConfigFileName = "ws_duels_config.json"
...
createCoordMarker(Vector3.new(-476.48, -6.28, 92.73), "L1", Color3.fromRGB(168, 168, 168))
createCoordMarker(Vector3.new(-483.12, -4.95, 94.80), "L END", Color3.fromRGB(148, 148, 148))
createCoordMarker(Vector3.new(-476.16, -6.52, 25.62), "R1", Color3.fromRGB(166, 166, 166))
createCoordMarker(Vector3.new(-483.04, -5.09, 23.14), "R END", Color3.fromRGB(133, 133, 133))
...
local StealData = {}
local DISCORD_TEXT = "discord.gg/877EsaWWa"
...
local function executeSteal(prompt, name)
    ...
    task.spawn(function()
        -- Método 1: callbacks internos (getconnections)
        local usedCallbacks = false
        ...
        -- Método 2: fireproximityprompt (fallback - funciona em qualquer executor)
        if not usedCallbacks then
            pcall(function()
                if fireproximityprompt then
                    fireproximityprompt(prompt)
                else
                    -- Último fallback: simular hold manualmente
...
local function enableGalaxySkyBright()
    ...
    galaxyCC.TintColor = Color3.fromRGB(200, 200, 200)
...
local C = {
    ...
    bgRow      = Color3.fromRGB(18, 18, 18),
    purple     = Color3.fromRGB(45, 45, 45),
    purpleLight= Color3.fromRGB(180, 180, 180),
    purpleDark = Color3.fromRGB(30, 30, 30),
...
end, Color3.fromRGB(168, 168, 168), 6)
...
end, Color3.fromRGB(166, 166, 166), 7)
...
print("[WS DUELS] Loaded - discord.gg/877EsaWWa")

-- ══════════════════════════════════════════════════════════
-- ── WS DUELS: SPEED ESP + DC LABEL ────────────────────────
-- ══════════════════════════════════════════════════════════
do
    local espGui = Instance.new("ScreenGui")
    espGui.Name = "WS_SpeedESP"
    espGui.ResetOnSpawn = false
    espGui.DisplayOrder = 99
    espGui.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")

    local espData = {} -- [player] = {bill, speedLbl, dcLbl}

    local function makeESP(player)
        if espData[player] then return end
        if player == Player then
            -- Para o próprio jogador: mostrar só speed em baixo da tela
            espData[player] = {self=true}
            return
        end

        local bb = Instance.new("BillboardGui")
        bb.Name = "WS_ESP_"..player.Name
        ...
        speedLbl.Position = UDim2.new(0,0,0,17)
        speedLbl.BackgroundTransparency = 1
        speedLbl.Font = Enum.Font.GothamBold
        speedLbl.TextSize = 11
        speedLbl.TextColor3 = Color3.fromRGB(255,220,80)
        speedLbl.Text = "speed: ?"
        speedLbl.TextStrokeTransparency = 0.5
        speedLbl.TextStrokeColor3 = Color3.new(0,0,0)

        -- DC label (discord link do WS)
        local dcLbl = Instance.new("TextLabel", bb)
        dcLbl.Size = UDim2.new(1,0,0,12)
        dcLbl.Position = UDim2.new(0,0,0,32)
        dcLbl.BackgroundTransparency = 1
        dcLbl.Font = Enum.Font.GothamMedium
        dcLbl.TextSize = 9
        dcLbl.TextColor3 = Color3.fromRGB(110,110,110)
        dcLbl.Text = "discord.gg/877EsaWWa"
        dcLbl.TextStrokeTransparency = 0.6
        dcLbl.TextStrokeColor3 = Color3.new(0,0,0)

        espData[player] = {bill=bb, speedLbl=speedLbl, dcLbl=dcLbl, nameLbl=nameLbl}
    end

    local function removeESP(player)
        local d = espData[player]
        if d and d.bill then
            pcall(function() d.bill:Destroy() end)
        end
        espData[player] = nil
    end

    -- Self speed HUD (canto inferior esquerdo)
    local selfGui = Instance.new("ScreenGui")
    selfGui.Name = "WS_SelfSpeed"
    selfGui.ResetOnSpawn = false
    selfGui.DisplayOrder = 98
    selfGui.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")

    local selfFrame = Instance.new("Frame", selfGui)
    selfFrame.Size = UDim2.new(0,130,0,32)
    selfFrame.Position = UDim2.new(0,8,1,-90)
    selfFrame.BackgroundColor3 = Color3.fromRGB(10,10,10)
    selfFrame.BackgroundTransparency = 0.15
    ...
    selfStroke.Color=Color3.fromRGB(50,50,50); selfStroke.Thickness=1; selfStroke.Transparency=0.2

    local selfNameLbl = Instance.new("TextLabel",selfFrame)
    selfNameLbl.Size=UDim2.new(1,0,0,13); selfNameLbl.Position=UDim2.new(0,8,0,3)
    selfNameLbl.BackgroundTransparency=1; selfNameLbl.Font=Enum.Font.GothamBold; selfNameLbl.TextSize=9
    selfNameLbl.TextColor3=Color3.fromRGB(160,160,160); selfNameLbl.TextXAlignment=Enum.TextXAlignment.Left
    selfNameLbl.Text = Player.DisplayName.." (você)"

    local selfSpeedLbl = Instance.new("TextLabel",selfFrame)
    selfSpeedLbl.Size=UDim2.new(1,0,0,14); selfSpeedLbl.Position=UDim2.new(0,8,0,14)
    selfSpeedLbl.BackgroundTransparency=1; selfSpeedLbl.Font=Enum.Font.GothamBlack; selfSpeedLbl.TextSize=11
    selfSpeedLbl.TextColor3=Color3.fromRGB(255,220,80); selfSpeedLbl.TextXAlignment=Enum.TextXAlignment.Left
    selfSpeedLbl.Text="speed: ?"
    ...
    game:GetService("RunService").Heartbeat:Connect(function()
        ...
        if selfChar then
            ...
            if hrp then
                ...
                selfSpeedLbl.Text = "speed: "..spd
            end
            if hum then selfSpeedLbl.TextColor3 = hum.WalkSpeed > 16 and Color3.fromRGB(255,180,50) or Color3.fromRGB(255,220,80) end
        end
        ...
        for player, data in pairs(espData) do
            ...
            local char = player.Character
            if not char then
                data.bill.Adornee = nil
                continue
            end
            local hrp = char:FindFirstChild("HumanoidRootPart")
            if not hrp then data.bill.Adornee = nil; continue end
            data.bill.Adornee = hrp
            local spd = math.floor(hrp.AssemblyLinearVelocity.Magnitude)
            data.speedLbl.Text = "speed: "..spd
            -- Colorir por velocidade
            data.speedLbl.TextColor3 = spd > 30 and Color3.fromRGB(255,100,80) or Color3.fromRGB(255,220,80)
        end
    end)

    -- Adicionar ESP para jogadores existentes e futuros
