...
local function ApplyDesync()
    ...
    if not ok or not Sync then return end
    ...
    for name, fn in pairs(Sync) do
        ...
        for idx, val in pairs(ups) do
            if typeof(val) == "function" and not isexecutorclosure(val) then
                ...
                if ok2 then
                    for i2, v2 in pairs(innerUps) do
                        if typeof(v2) == "boolean" then
                            -- Neutraliza a função wrapper e zera os upvalues booleanos internos
                            pcall(debug.setupvalue, fn, idx, newcclosure(function() end))
                            pcall(debug.setupvalue, val, i2, false)
                        end
                    end
                end
            end
...
-- Anda até encostar na parede mais próxima antes do clone (evita erro de posição)
local function WalkIntoWall(maxDist, timeout)
    ...
    for _, dir in ipairs(dirs) do
        ...
        local result = workspace:Raycast(hrp.Position, flat * maxDist, params)
    ...
    if bestDir then
        local target = hrp.Position + bestDir * math.max(bestDist - 0.4, 0)
        hum:MoveTo(target)
        local deadline = tick() + timeout
        repeat task.wait(0.05)
        until (hrp.Position - target).Magnitude < 0.6 or tick() > deadline
    else
        -- Sem parede próxima: dá um passo curto pra frente mesmo assim
        hum:MoveTo(hrp.Position + hrp.CFrame.LookVector * 1.5)
        task.wait(0.3)
    end
end
...
-- ── WS MOBILE DOCK (barra horizontal na base) ────────────────────────
if IS_MOBILE then
    task.spawn(function()
        task.wait(2)

        local dockGui = Instance.new("ScreenGui")
        dockGui.Name = "WsMobileDock"
        dockGui.ResetOnSpawn = false
        dockGui.DisplayOrder = 55
        dockGui.IgnoreGuiInset = true
        dockGui.Parent = PlayerGui

        -- Constantes visuais do dock
        local DOCK_H     = 72
        local DOCK_W_PCT = 0.94  -- 94% da largura da tela
        local BTN_W      = 58
        local BTN_H_D    = 52
        local CORNER     = UDim.new(0, 14)
        local BG_DOCK    = Color3.fromRGB(12, 12, 12)
        local ACC        = Color3.fromRGB(200, 200, 200)

        -- Frame principal do dock — ancorado na base da tela
        local dock = Instance.new("Frame", dockGui)
        dock.Name = "Dock"
        dock.AnchorPoint = Vector2.new(0.5, 1)
        dock.Position = UDim2.new(0.5, 0, 1, -10)
        dock.Size = UDim2.new(DOCK_W_PCT, 0, 0, DOCK_H)
        dock.BackgroundColor3 = BG_DOCK
        dock.BackgroundTransparency = 0.12
        dock.BorderSizePixel = 0
        dock.Active = true
        Instance.new("UICorner", dock).CornerRadius = UDim.new(0, 20)
        ...
        -- Arrastar dock verticalmente (para subir da base se precisar)
        local dockDragStart, dockStartY, dockDragging, dockDidDrag
        dock.InputBegan:Connect(function(inp)
            if inp.UserInputType ~= Enum.UserInputType.Touch then return end
            dockDragging = true
            dockDidDrag = false
            dockDragStart = inp.Position
            dockStartY = dock.Position.Y.Scale
        end)
        UserInputService.InputChanged:Connect(function(inp)
            if not dockDragging then return end
            if inp.UserInputType ~= Enum.UserInputType.Touch then return end
            local dy = (inp.Position.Y - dockDragStart.Y) / dockGui.AbsoluteSize.Y
            if math.abs(dy) > 0.01 then dockDidDrag = true end
            local newY = math.clamp(dockStartY + dy, 0.3, 1)
            dock.Position = UDim2.new(0.5, 0, newY, -10)
        end)
        UserInputService.InputEnded:Connect(function(inp)
            if inp.UserInputType == Enum.UserInputType.Touch then
                dockDragging = false
        ...
        -- Helper: cria botão no dock
        local function dockBtn(icon, label, accentColor, onTap)
            accentColor = accentColor or ACC

            local frame = Instance.new("Frame", dock)
            frame.Size = UDim2.new(0, BTN_W, 0, BTN_H_D)
            ...
            subLbl.Position = UDim2.new(0, 2, 1, -16)
            ...
            -- Touch: distingue tap de scroll/drag do dock
            local touchStart, tapped
            frame.InputBegan:Connect(function(inp)
                if inp.UserInputType ~= Enum.UserInputType.Touch then return end
                touchStart = inp.Position
                tapped = true
                TweenService:Create(frame, TweenInfo.new(0.07), {BackgroundTransparency = 0.55}):Play()
                TweenService:Create(fStroke, TweenInfo.new(0.07), {Transparency = 0.1}):Play()
            end)
            frame.InputChanged:Connect(function(inp)
                if inp.UserInputType ~= Enum.UserInputType.Touch then return end
                if touchStart and (inp.Position - touchStart).Magnitude > 14 then
                    tapped = false
                end
            end)
            frame.InputEnded:Connect(function(inp)
                ...
                TweenService:Create(fStroke, TweenInfo.new(0.15), {Transparency = 0.55}):Play()
                if tapped and not dockDidDrag then
            ...
            frame.MouseButton1Click:Connect(function()
                if not IS_MOBILE then task.defer(function() pcall(onTap) end) end
            end)

            return frame, iconLbl, fStroke
        end

        -- ── BOTÃO TP ─────────────────────────────────────────────────────
        dockBtn("⚡", "TP", Color3.fromRGB(100, 180, 255), function()
            if _G.tpToBestBrainrot then pcall(_G.tpToBestBrainrot)
            else pcall(runAutoSnipe) end
        end)

        -- ── BOTÃO CLONE ──────────────────────────────────────────────────
        dockBtn("📋", "CLONE", Color3.fromRGB(130, 220, 130), function()
            local char = LocalPlayer.Character
            local hrp  = char and char:FindFirstChild("HumanoidRootPart")
            local inBase = false
            if hrp then
                ...
                if plots then
                    for _, plot in ipairs(plots:GetChildren()) do
                        ...
                        if yb and yb:IsA("BillboardGui") and yb.Enabled then
                            ...
                            if ok and dist < 120 then inBase = true; break end
                        end
                    end
                end
            end
        ...
        -- ── INSTA RESET ──────────────────────────────────────────────────
        dockBtn("💀", "RESET", Color3.fromRGB(230, 80, 60), function()
            executeReset()
        end)

        -- ── CARPET SPEED ─────────────────────────────────────────────────
        local _, carpetIcon, carpetStroke = dockBtn("🛸", "CARPET", Color3.fromRGB(200, 160, 255), function()
        ...
        -- Atualiza visual conforme estado do carpet
        task.spawn(function()
            while carpetIcon and carpetIcon.Parent do
                local on = State.carpetSpeedEnabled
                carpetIcon.TextColor3 = on and Color3.fromRGB(255, 220, 80) or Color3.fromRGB(200, 160, 255)
                if carpetStroke then
                    carpetStroke.Color = on and Color3.fromRGB(255, 220, 80) or Color3.fromRGB(200, 160, 255)
                    carpetStroke.Transparency = on and 0.05 or 0.55
                end
                task.wait(0.3)
            end
        end)
        ...
        -- ── INVISIBLE STEAL ───────────────────────────────────────────────
        local _, invisIcon, invisStroke = dockBtn("👁", "INVIS", Color3.fromRGB(200, 200, 200), function()
            if _G.toggleInvisibleSteal then pcall(_G.toggleInvisibleSteal) end
        end)
        task.spawn(function()
            while invisIcon and invisIcon.Parent do
                local on = _G.invisibleStealEnabled
                invisIcon.Text = on and "🚫" or "👁"
                invisIcon.TextColor3 = on and Color3.fromRGB(255, 80, 80) or Color3.fromRGB(200, 200, 200)
                if invisStroke then invisStroke.Transparency = on and 0.05 or 0.55 end
                task.wait(0.3)
            end
        end)

        -- ── SETTINGS ─────────────────────────────────────────────────────
        dockBtn("⚙", "CFG", Color3.fromRGB(180, 180, 180), function()
            if _G.BullysSettingsUI and _G.BullysSettingsUI.panel then
...
function buildMiniActionsUI()
    ...
    -- Botão de Reset instantâneo
    do
        local resetBtn = Instance.new("TextButton", cont)
        resetBtn.Size = UDim2.new(1, 0, 0, BTN_H)
        resetBtn.BackgroundColor3 = Color3.fromRGB(190, 70, 30)
        ...
        resetBtn.LayoutOrder = 43
        Instance.new("UICorner", resetBtn).CornerRadius = UDim.new(0, 7)
        local resetStroke = Instance.new("UIStroke", resetBtn)
        resetStroke.Color = Color3.fromRGB(230, 100, 50)
        resetStroke.Thickness = 1
        resetStroke.Transparency = 0.4
        -- Feedback visual ao apertar
        resetBtn.MouseButton1Click:Connect(function()
            TweenService:Create(resetBtn, TweenInfo.new(0.07), {BackgroundTransparency = 0.5}):Play()
            task.delay(0.15, function()
                TweenService:Create(resetBtn, TweenInfo.new(0.15), {BackgroundTransparency = 0.05}):Play()
            end)
            executeReset()
        end)
